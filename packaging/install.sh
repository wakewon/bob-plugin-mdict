...
if [ "${BOB_MDICT_INSTALL_SMOKE:-0}" != "1" ] && launchctl list 2>/dev/null | awk -v label="$LABEL" '$3 == label { found = 1 } END { exit !found }'; then
    ...
    for _ in $(seq 1 50); do
        launchctl list 2>/dev/null | awk -v label="$LABEL" '$3 == label { found = 1 } END { exit !found }' || break
        sleep 0.2
    done
    if launchctl list 2>/dev/null | awk -v label="$LABEL" '$3 == label { found = 1 } END { exit !found }'; then
